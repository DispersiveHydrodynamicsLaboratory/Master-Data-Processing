...
process_files = 0; % Saves Timestamps
...
load_cropped  = 1; % Uses cropped images instead of reprocessing originals
check_scale   = 0; % Check formatting of the video image

data_dir = '/Volumes/Data Storage/Marketing_Videos_ST3/';
trialnum = 12;
if trialnum == 13;
    extra_dir = ['/full_cam/SoliSec_Five/'];
else
    extra_dir = ['/full_cam/'];
end
source_dir = [data_dir,'Trial',num2str(trialnum),extra_dir];
...
rotation = -33;  % Rotation angle to apply to all images (-33)
...
if find_rect
    ...
    get_rect_horiz(trialnum,data_dir,extra_dir,1,1,5,hfac,rotation,1);
